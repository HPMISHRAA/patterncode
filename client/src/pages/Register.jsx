...
import { Mail, Lock, Code, ShieldAlert, User } from 'lucide-react';

export default function Register() {
  const { user, register, loginWithGoogle, error, isMockAuth } = useAuth();
      ...
      <div className="w-full max-w-md glass-card p-8 rounded-2xl flex flex-col gap-6 shadow-xl border border-obsidian-850">
        ...
        <div className="text-center flex flex-col items-center gap-2">
          ...
          <p className="text-obsidian-400 text-xs">
            Start solving and tracking streaks.
          </p>
        </div>
        ...
        {isMockAuth && (
          <div className="p-3 rounded-lg bg-gold-500/10 border border-gold-500/20 text-gold-500 text-xs leading-relaxed">
            <span className="font-bold">Offline Mock Auth Active:</span> Enter any user details to mock register and sync locally.
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-obsidian-300">Display Name</label>
            <div className="relative">
              <User className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-obsidian-500" />
              <input 
                type="text" 
                value={name}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-obsidian-300">Confirm Password</label>
            <div className="relative">
              <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-obsidian-500" />
              <input 
                type="password" 
        ...
        <p className="text-center text-xs text-obsidian-400 mt-2">
          Already registered?{' '}
          <Link to="/login" className="font-semibold text-gold-500 hover:text-gold-400">
            Sign In
          </Link>
        </p>
